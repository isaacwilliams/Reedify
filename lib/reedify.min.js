...
this.reedify.templates.feed_item = Handlebars.template(function(Handlebars, depth0, helpers, partials, data) {
    ...
    var AppRouter, ChangesPopupView, EditStreamPopupView, FeedItem, FeedItemList, FeedList, LoginPopupView, ManageFeedsPopupView, Navigation, PopupView, ReedifyApp, UserData, Wrangler, app, __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
        function ctor() {
    ...
    FeedList = function() {
        ...
        PopupView.prototype.animationHide = "popup-hide", PopupView.prototype.showPopup = function(callback) {
            return $("body").addClass("noscroll"), this.$el.animate(this.animationShow, {
                duration: 250,
                easing: "ease-in-out",
                complete: function() {
            ...
            var _this = this;
            return $("body").removeClass("noscroll"), this.$el.animate(this.animationHide, {
                duration: 250,
                easing: "ease-in-out",
                complete: function() {
                    return _this.$el.remove(), callback ? callback() : void 0;
                }
            });
        }, PopupView;
        ...
        function LoginPopupView() {
            ...
            this.$el = $(this.template({
                version: app.version
            })), loginPopup = this, this.$el.find("form").on("submit", function() {
                var email, password;
                return email = loginPopup.$el.find('input[name="email"]').val(), password = loginPopup.$el.find('input[name="password"]').val(), 
                loginPopup.$el.addClass("loading"), app.user.login(email, password, function() {
                    return loginPopup.hidePopup();
                }, function(data) {
                    return (null != data ? data.error : void 0) && loginPopup.$el.find(".error").html(data.error), 
                    loginPopup.$el.removeClass("loading");
                }), !1;
            });
        }
        return __extends(LoginPopupView, _super), LoginPopupView.prototype.template = window.reedify.templates.user_login, 
        LoginPopupView.prototype.animationShow = "login-popup-show", LoginPopupView;
    }(PopupView), ChangesPopupView = function(_super) {
        function ChangesPopupView() {
            var _this = this;
            this.$el = $(this.template(this)), $.ajax({
                url: "/assets/readme.html",
                success: function(data) {
                    return _this.$el.find("div.content div.about-content").html(data), _this.showPopup();
                }
            });
        }
        return __extends(ChangesPopupView, _super), ChangesPopupView.prototype.template = window.reedify.templates.popup_about, 
        ChangesPopupView.prototype.animationShow = "login-popup-show", ChangesPopupView;
    }(PopupView), ManageFeedsPopupView = function(_super) {
        function ManageFeedsPopupView() {
            var $el, managePopup;
            managePopup = this, this.$el = $el = $(this.template(window.app.feedList)), this.showPopup(), 
            this.$el.find("a.close-button").on("click", function(e) {
                return e.preventDefault(), managePopup.hidePopup();
            }), this.$el.find("a.feed-unsubscribe").on("click", function(e) {
                var $listItem, feedId;
                return e.preventDefault(), feedId = parseInt($(this).attr("rel")), $listItem = $el.find("#feed-" + feedId), 
                $listItem.css({
                    opacity: .5
        ...
        return __extends(ManageFeedsPopupView, _super), ManageFeedsPopupView.prototype.template = window.reedify.templates.popup_feed_edit, 
        ManageFeedsPopupView;
    }(PopupView), EditStreamPopupView = function(_super) {
        function EditStreamPopupView(streamId) {
            var $el, editStream, filterCache, selectedFeedIds, selectedFeeds, streamPopup, _feed, _i, _j, _len, _len1, _ref, _ref1, _stream, _this = this;
            if (editStream = {}, streamId) for (_ref = window.app.nav.streams.streams, _i = 0, 
            _len = _ref.length; _len > _i; _i++) _stream = _ref[_i], _stream.stream_id === streamId && (editStream = _stream);
            if (editStream.feeds) {
                for (selectedFeedIds = [], _ref1 = editStream.feeds, _j = 0, _len1 = _ref1.length; _len1 > _j; _j++) _feed = _ref1[_j], 
            ...
            this.all_feeds = editStream.all_feeds, this.feeds = editStream.feeds, this.only_unread = editStream.only_unread, 
            this.stream_id = editStream.stream_id, this.title = editStream.title, this.search_term = editStream.search_term, 
            streamPopup = this, this.$el = $el = $(this.template(this)), this.showPopup(), (!this.stream_id || this.all_feeds) && (this.$el.find('input[name="all_feeds"]').attr("checked", !0), 
            this.$el.find("div.include-only").addClass("hide")), this.$el.find('input[name="all_feeds"]').on("change", function() {
                return $(this)[0].checked ? $el.find("div.include-only").addClass("hide") : $el.find("div.include-only").removeClass("hide");
            }), this.$el.find("a.cancel").on("click", function(e) {
                return e.preventDefault(), _this.hidePopup();
            }), this.$el.find("a.destroy-feed").on("click", function(e) {
                var options;
                return e.preventDefault(), $el.addClass("loading"), options = {
                    stream_id: streamPopup.stream_id
                }, app.wrangler.query("streams/destroy", options, function() {
                    return window.app.nav.loadStreams(!0), streamPopup.hidePopup();
                }, function(data) {
                    return console.log(data);
                });
            }), filterCache = [], this.$el.find("li.list-feed-item").each(function() {
                return filterCache.push({
                    string: $(this).text().toLowerCase(),
                    dom: $(this)
                });
            }), this.$el.find("div.filter input.filter").on("keyup click search", function() {
                ...
                return e.preventDefault(), $el.find("input.feed-checkbox").attr("checked", !0);
            }), this.$el.find("div.filter a.filter-select-none").on("click", function(e) {
                return e.preventDefault(), $el.find("input.feed-checkbox").removeAttr("checked");
            }), this.$el.find("form").on("submit", function(e) {
                var data, feedId, feedsSelected, options, queryURL, _data, _k, _len2;
                for (e.preventDefault(), $el.addClass("loading"), data = $(this).serializeArray(), 
                ...
                app.wrangler.query(queryURL, options, function() {
                    return window.app.nav.loadStreams(!0), $el.removeClass("loading"), streamPopup.hidePopup();
                }, function(data) {
                    return console.log(data), window.app.nav.loadStreams(!0), $el.removeClass("loading"), 
                    streamPopup.hidePopup();
                }), !1;
            });
        }
        return __extends(EditStreamPopupView, _super), EditStreamPopupView.prototype.template = window.reedify.templates.popup_stream_edit, 
        EditStreamPopupView;
        ...
        UserData.prototype.setAccessToken = function(accessToken) {
            ...
            var loginPopup;
            return loginPopup = new LoginPopupView(), $("body").append(loginPopup.$el), loginPopup.showPopup();
        ...
        FeedItem.prototype.select = function() {
            ...
            var body, _ref, _this = this;
            return -1 !== (null != (_ref = this.body) ? _ref.indexOf("script") : void 0) && (body = $(this.body), 
        ...
        FeedItemList.prototype.clear = function() {
            ...
            for ($itemList = this.$el.find("div.item-list"), _ref = this.feedItems, _results = [], 
            _i = 0, _len = _ref.length; _len > _i; _i++) feedTtem = _ref[_i], feedTtem.$el ? _results.push(void 0) : _results.push($itemList.append(feedTtem.render()));
            return _results;
            ...
            this.stream_id && (queryURL = "streams/stream_items"), this.current_request = app.wrangler.query(queryURL, options, function(data) {
                ...
                for (_this.setLoading(!1), console.log(data, data.count), _ref = data.feed_items, 
                _i = 0, _len = _ref.length; _len > _i; _i++) feed_item = _ref[_i], feedItem = new FeedItem(feed_item, _this), 
        ...
        Navigation.prototype.render = function() {
            ...
            var manangeFeeds;
            return manangeFeeds = new ManageFeedsPopupView(), $("body").append(manangeFeeds.$el);
            ...
            var editStreamPopup;
            return app.disableControls(), editStreamPopup = new EditStreamPopupView(streamId), 
        ...
        return AppRouter.prototype.urlBase = "", AppRouter.prototype.start = function() {
            ...
            for (path = path.replace(this.urlBase, ""), _ref = this.routes, _results = [], _i = 0, 
            _len = _ref.length; _len > _i; _i++) route = _ref[_i], match = route.path.exec(path), 
            ...
            var changesPopup;
            return changesPopup = new ChangesPopupView(), $("body").append(changesPopup.$el);
